...
bool Orbit_In_Progress = false;	// defines whether an orbit drive is in progress
int Orbit_Start_Orientation = 0;	// used to adjust drive angle during orbit
int Orbit_Speed = 0;
...
task Lifter()
{
	...
	while(true)
	{
		...
		case HIGH_LIFT:
			...
			motor[lifterMotor2] = -LIFTER_SPEED_UP_FAST;
		...
		case MANUAL_DOWN:
			if(lifterEnd !=true)
			{
				...
				motor[lifterMotor2] = LIFTER_SPEED_DOWN;
			}
			else
			{
				motor[lifterMotor] = 0;
		...
		case OVERRIDE_DOWN:
			...
			motor[lifterMotor2] = LIFTER_SPEED_DOWN_FAST;
			break;
		}
...
void initializeRobot()
{
	...
	moveLightServo(UP);															// reset light sensor into UP position for safety
	wait1Msec(400);
	return;
}
...
task main()
{
	...
	{											// run first read of joystick to verify connectivity
		...
		else if(joy1Btn(7)) hypoteneuse = hypoteneuse/(float)3;  // button seven gives slow slow speed
		else hypoteneuse = hypoteneuse/(float)1.4;		// otherwise default is half speed
			//---------------------thisone
		...
		//===============================================================================================
		// Code to handle orbit drive request from driver joystick
		//===============================================================================================
		if (joy1Btn(10))		// driver wants to orbit
		{
			if (!Orbit_In_Progress)											// this is a new button press to start orbiting
			{
				Orbit_Speed = 30;												// this is for now only
				Orbit_In_Progress=true;									// setup to say we are in action
				ClearTimer(T2);													// reset the timer so we can calculate the length of the orbit
				Orbit_Start_Orientation = constHeading+90;	// this is a reference of the orientation when we started orbiting
				drive_speed=Orbit_Speed;								// setup the speed that we are going to use when orbiting
				drive_angle=Orbit_Start_Orientation;		// and configure the rbot orientation that we'll begin with
			}
			else
			{
				int current_angle = (time1(T2)/30)+Orbit_Start_Orientation;		// add one degree every 30mS
				int rotation_error = (constHeading+90)-current_angle;
				int rotator = 0;
				if (rotation_error < -3) rotator = -10;
				if (rotation_error > 3) rotator = 10;
				rotation_factor=rotator;								// calculate desired versus actual to get a rotation factor
				drive_angle=current_angle; 							// the drive angle adjusts as we rotate
			}
		}
		else
		{
			Orbit_In_Progress = false;	// button has been released so show that nothing is in progress
			// NOTE: drive_speed does not get zeroed here - if nothing is pressed then speed will be zero anyhow
		}
		//========================================================================================
		// Now decide if we'll use field relative or robot relative drive
		//========================================================================================
		float gyro_angle=0;																			// assume we'll be using robot relative mode (default)
		if(joy1Btn(11) == true) gyro_angle = constHeading;			// configure field relative drive if requested
			if (Orbit_In_Progress) gyro_angle = constHeading;				// Orbit always works using field relative mode
