...
task sensors()
{
	...
	while(true)
	{
		...
		if ((num!= -1) && (num!=255))
		{if (num>=0 && num<=127) sonarLive = num;
			if (num<0) sonarLive=256+num;
		}
		if ((num2!= -1) && (num2!=255))
		{
			if (num2>=0 && num2<=127) sonarLive2 = num2;
			if (num2<0) sonarLive2=256+num2;
		}
...
void GyroSonar_moveV2(int time, int WhichSensors, int distanceX, int distanceY, int power,bool StopWhenDone, bool adjust, bool ConstOrRel)
{
	...
	while(!Done)
	{
		if(WhichSensors != 2)										//----------------
		{																				//
			if(sonarLive2 > distanceY)

			{if (sonarLive2!=lastvalue)
				{count++;				// Read 2nd sonar sensor and tell distance
					lastvalue=sonarLive2;
				}
			}
			else count=0;
			if(count > 2) Done = true;								// from base wall, tripping loop accordingly
		}																				//
		else if(time1[T1] > time) Done = true;	//----------------

		if(adjust == true)
		{
			if(WhichSensors != 1) sonar_adj = ((distanceX - sonarLive)*3);
			...
			motor[LB_motor] = -(current_power + adj_power - sonar_adj);
			motor[RB_motor] = current_power + adj_power + sonar_adj;
		}
	}
	if(StopWhenDone==true)
	{
...
task main()
{
	...
	case 2:
		...
		GyroTime_moveV2(5000, 60,true, true, false);
