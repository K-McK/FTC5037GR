...
task datalog()
{
	...
	while(LogData)						// add samples to the file
	{
		...
		WriteText(IRFileHandle, IRIoResult, sString);
		StringFormat(sString, "%7d", sonarlive2);				//	Side mounted sonar
		WriteText(IRFileHandle, IRIoResult, sString);
		StringFormat(sString, "%7d", 	sonarRaw2);				//	Front mounted sonar
		WriteText(IRFileHandle, IRIoResult, sString);
...
void GyroTimeS_moveV2(int time, int power, bool light,bool StopWhenDone, bool adjust, bool ConstOrRel)
{
	...
	while(!Done)
	{
		...
		if(light == true)
		{
			if(light_normalised > light_threshold) i++;//go here 1
			if(i > 2) Done = true;			// was 10, but that may be too high, now 5 at 21 Nov 2012
		}
...
bool GyroSonar_moveV2(int time, int WhichSensors, int distanceX, int distanceY, int power,bool StopWhenDone, bool adjust, bool ConstOrRel)
{
	...
	{
		mycount = count;
		if(WhichSensors != SIDE)								// anything except side only adjustments
		{																				//
...
int position1 ()//go here
{
	...
	leftAngle = 60;
	int column = 0;
	GyroSonar_moveV2(0, SIDE_BACK, 102, 118, -60,true, true, CONSTANT);//(int time, int WhichSensors, int distanceX, int distanceY, int power,bool StopWhenDone, bool adjust, bool ConstOrRel
	StopRobot();
	column = IR_Bearing;
	StopRobot();
	wait1Msec(200);
	...
	wait1Msec(200);
	GyroTimeS_moveV2(1200,40,false,true,false,CONSTANT);
	StopRobot();

	moveLightServo(DOWN);
	wait1Msec(1000);
	bool sawLine = Gyro_TurnV2(45,-10,CONSTANT);
	...
	GyroTimeS_moveV2(60,18,true,false,false,false);
	GyroTime_moveV2(1200,-40,true,false,false);
