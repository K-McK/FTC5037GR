...
task datalog()
{
	...
	while(LogData)						// add samples to the file
	{
		...
		WriteText(IRFileHandle, IRIoResult, sString);
		StringFormat(sString, "%7d", motor[LF_motor]);				//	Front mounted sonar
		strcat(sString,CRLF);
		WriteText(IRFileHandle, IRIoResult, sString);
...
int position1 ()
{
	...
	leftAngle = 60;
	int column = 0;
	GyroSonar_moveV2(0, SIDE_BACK, 106, 124, -38,true, true, CONSTANT);//(int time, int WhichSensors, int distanceX, int distanceY, int power,bool StopWhenDone, bool adjust, bool ConstOrRel)
	column = IR_Bearing;
	GyroTimeS_moveV2(790,40,false,true,true,CONSTANT);
	moveLightServo(DOWN);
	wait1Msec(1000);
	...
	if(sawLine)
	{
		...
		//GyroTimeS_moveV2(8000,-15,true,true,false,false);
		GyroDrive(8000,rightAngle,38,true,true);
		GyroDrive(8000,leftAngle,25,true,true);
	}
	else
	{
		//GyroTimeS_moveV2(8000,-15,true,true,false,false);
		//GyroTimeS_moveV2(8000,18,true,true,false,false);
		GyroDrive(8000,leftAngle,38,true,true);
		GyroDrive(8000,rightAngle,25,true,true);
	}
	GyroTimeS_moveV2(30,-15,true,false,false,false);
