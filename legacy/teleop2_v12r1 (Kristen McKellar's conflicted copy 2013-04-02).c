...
task main()
{
	...
	{											// run first read of joystick to verify connectivity
		...
		if (joy1Btn(10))		// driver wants to orbit
		{
			...
			{
				Orbit_Speed = 50;												// this is for now only
				...
				Orbit_Start_Orientation = constHeading;	// this is a reference of the orientation when we started orbiting
			...
			else
			{
				...
				rotation_factor=(constHeading-current_angle)*ROTATION_GAIN*2;		// calaculate desired versus actual to get a rotation factor
		...
		speed_limit-=abs(rotation_factor);								// adjuist speed limit down to allow for rotation (if any)
